...
describe("uniswap/quote/getUniswapRouteV4.test.ts", function () {
    ...
    test("ETH -> L3 with Veraswap fee", async () => {
    ...
    test("ETH -> L3 with Veraswap and referral fee", async () => {
        const currencyIn = zeroAddress; // ETH
        const currencyOut = tokenL3;
        const currencyHops: Address[] = [];
    ...
    test("L3 -> WETH", async () => {
        ...
        const currencyOut = contracts.weth9;
        const currencyHops: Address[] = [];
        ...
        const currencyInBalanceBeforeSwap = await getBalance(currencyIn);
        // const currencyOutBalanceBeforeSwap = await getBalance(currencyOut);
        const currencyOutBalanceBeforeSwap = await getBalance(contracts.weth9);
    ...
    test("L3 -> ETH", async () => {
        ...
        // Same as calling getUniswapRouteExactIn
        // const metaQuote = await opChainL1Client.readContract({
