...
library V3QuoterUtils {
    ...
    function getDeployBytecode(address factory, bytes32 poolInitCodeHash) internal pure returns (bytes memory) {
        return abi.encodePacked(type(V3Quoter).creationCode, abi.encode(factory, poolInitCodeHash));
    }
    ...
    function getOrCreate2(address factory, bytes32 poolInitCodeHash) internal returns (address addr, bool exists) {
        ...
        if (!exists) {
            address deployed = address(new V3Quoter{salt: Create2Utils.BYTES32_ZERO}(factory, poolInitCodeHash));
