...
library V3PositionManagerMockUtils {
    Vm private constant vm = Vm(address(uint160(uint256(keccak256("hevm cheat code")))));

    function getDeployBytecode(address factory, bytes32 poolInitCodeHash) internal pure returns (bytes memory) {
        return abi.encodePacked(type(V3PositionManagerMock).creationCode, abi.encode(factory, poolInitCodeHash));
    }
    ...
    function getOrCreate2(address factory, bytes32 poolInitCodeHash) internal returns (address addr, bool exists) {
        ...
        if (!exists) {
            address deployed = address(
                new V3PositionManagerMock{salt: Create2Utils.BYTES32_ZERO}(factory, poolInitCodeHash)
            );
