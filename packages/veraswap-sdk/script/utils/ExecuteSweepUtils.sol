...
library ExecuteSweepUtils {
    Vm private constant vm = Vm(address(uint160(uint256(keccak256("hevm cheat code")))));

    function getDeployBytecode() internal pure returns (bytes memory) {
        return abi.encodePacked(type(ExecuteSweep).creationCode);
    }
    ...
    function getOrCreate2() internal returns (address addr, bool exists) {
        ...
        if (!exists) {
            address deployed = address(new ExecuteSweep{salt: Create2Utils.BYTES32_ZERO}());
