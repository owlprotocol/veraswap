...
contract MetaQuoterV2V4Test is MetaQuoterBaseTest {
    ...
    function test_V2V4_A_L4_B() public {
        ...
        path[0] = currencyIn;
        path[1] = quote.path[0].intermediateCurrency;
        ...
        bytes memory routerCommands = abi.encodePacked(uint8(Commands.V2_SWAP_EXACT_IN), uint8(Commands.V4_SWAP));
    ...
    function test_V2V4_ETH_B_A() public {
        ...
        // Encode V2 Swap
        Currency[] memory path = new Currency[](2);
        path[0] = _mapCurrencyToWeth9(currencyIn);
        path[1] = quote.path[0].intermediateCurrency;
        bytes memory v2Swap = abi.encode(
            ActionConstants.ADDRESS_THIS, // recipient
            amount,
        ...
        routerCommandInputs[0] = abi.encode(ActionConstants.ADDRESS_THIS, ActionConstants.CONTRACT_BALANCE); // Wrap ETH to WETH (input to v2 swap)
    ...
    function test_V2V4_A_ETH_B() public {
        ...
        Currency[] memory path = new Currency[](2);
        path[0] = currencyIn;
        path[1] = _mapCurrencyToWeth9(quote.path[0].intermediateCurrency);
        bytes memory v2Swap = abi.encode(
            ActionConstants.ADDRESS_THIS, // recipient
            amount,
            0, // amountOutMinimum ignored for intermediate swap
    ...
    function test_V4V2_A_B_ETH() public {
        ...
        Currency[] memory path = new Currency[](2);
        path[0] = quote.path[0].intermediateCurrency;
        path[1] = _mapCurrencyToWeth9(quote.path[1].intermediateCurrency);
        bytes memory v2Swap = abi.encode(
            ActionConstants.ADDRESS_THIS, // recipient
            ActionConstants.CONTRACT_BALANCE, // amountIn = contract balance
            quote.variableAmount, // amountOutMinimum
        ...
        routerCommandInputs[0] = abi.encode(v4Actions, v4ActionParams);
        routerCommandInputs[1] = v2Swap;
        routerCommandInputs[2] = abi.encode(ActionConstants.MSG_SENDER, quote.variableAmount); // Unwrap WETH to ETH (input to v2 swap)
        // Execute Swap
        uint256 currencyInBalanceBeforeSwap = currencyIn.balanceOf(msg.sender);
        uint256 currencyOutBalanceBeforeSwap = currencyOut.balanceOf(msg.sender);
        uint256 deadline = block.timestamp + 20;
    ...
    function test_V4V2_A_ETH_B() public {
        ...
        // Encode V2 Swap
        Currency[] memory path = new Currency[](2);
        path[0] = _mapCurrencyToWeth9(quote.path[0].intermediateCurrency);
        path[1] = quote.path[1].intermediateCurrency;
        bytes memory v2Swap = abi.encode(
            ActionConstants.MSG_SENDER, // recipient
            ActionConstants.CONTRACT_BALANCE, // amountIn = contract balance
        ...
        bytes[] memory routerCommandInputs = new bytes[](3);
        routerCommandInputs[0] = abi.encode(v4Actions, v4ActionParams);
        routerCommandInputs[1] = abi.encode(ActionConstants.ADDRESS_THIS, ActionConstants.CONTRACT_BALANCE); // Wrap ETH to WETH (input to v2 swap)
        routerCommandInputs[2] = v2Swap;
        // Execute Swap
        uint256 currencyInBalanceBeforeSwap = currencyIn.balanceOf(msg.sender);
        uint256 currencyOutBalanceBeforeSwap = currencyOut.balanceOf(msg.sender);
        uint256 deadline = block.timestamp + 20;
