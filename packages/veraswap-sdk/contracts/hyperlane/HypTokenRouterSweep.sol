...
/// @notice This contract can only be used with ERC20 tokens, not native tokens
...
contract HypTokenRouterSweep is Sweep {
    error BalanceZero();
    ...
    function transferRemote(
        ...
        address hook
    ) external payable isNotLocked returns (bytes32 messageId) {
        uint256 balance = TokenRouter(router).balanceOf(address(this));
        if (balance == 0) revert BalanceZero();
