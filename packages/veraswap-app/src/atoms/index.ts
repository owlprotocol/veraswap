...
import {
  balanceOf as balanceOfAbi,
  allowance as allowanceAbi,
} from "@owlprotocol/veraswap-sdk/artifacts/IERC20";
...
export const quoteAtom = atomWithQuery((get) => {
  ...
  const exactCurrencyAmount =
    tokenIn && tokenInAmount
      ...
      : emptyCurrencyAmount;
  const quoterAddress = chainId
    ? UNISWAP_CONTRACTS[chainId].QUOTER
    : zeroAddress;
  ...
  AtomWithQueryResult<[bigint, bigint], Error>,
  [],
  void
>;
...
export const swapStepAtom = atom((get) => {
  ...
  if (hash && hash != receipt.data?.transactionHash)
  ...
  if (tokenInBalance === null || tokenInBalance < tokenInAmount)
    return SwapStep.INSUFFICIENT_BALANCE;
  if (
    tokenInPermit2Allowance === null ||
    tokenInPermit2Allowance < tokenInAmount
  )
