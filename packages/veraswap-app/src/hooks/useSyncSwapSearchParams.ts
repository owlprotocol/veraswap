...
import { tokenInAtom, tokenOutAtom, networkTypeAtom } from "@/atoms/index.js";

export function useSyncSwapSearchParams(allTokens: VeraSwapToken[]) {
    const search = useSearch({ from: "/" });
    const [, setTokenIn] = useAtom(tokenInAtom);
    const [, setTokenOut] = useAtom(tokenOutAtom);
    const [, setNetworkType] = useAtom(networkTypeAtom);
